...
`questions.ts` で大量の問題データを一括管理する。今後も追加・削除・差し替えが適宜行われる前提で、問題の順序や件数に依存しない実装にすること。部屋選択時にランダムで出題セットを作成し、一部の部屋では選択肢をシャッフルして出題する（設定は `src/pages/Lobby.tsx` の `roomConfigs` に集約）。
出題時の選択肢表示は `a`〜`e` のラベルを付与する。ラベルは表示順（配列インデックス）に基づいて生成し、選択肢シャッフル後も整合するようにする（表示・正答表示の双方で同様の処理）。

以下の形式でデータを保持する。`correctAnswers` は配列で保持し、複数回答に対応する。`sourceId` は出典IDで、例として `119A75` は「第119回医師国家試験の A 問題・75 問目」を意味する。第119回の正答一覧は `public/119_answers.txt` に保存してあるので参照に使うこと。

## questions インポート指針

`src/data/questions.ts` に大量の問題を追加・更新する際は、以下の様式を厳守すること。

1. `options[].text` から選択肢記号の `a`〜`e` は取り除く（表示側で `a.`〜`e.` を付与する）。  
2. 組み合わせ問題は対応関係が分かるように `---` を挿入する。  
   例: `Ménière 病 --- 難　聴` / `SGLT2 阻害薬 --- 近位尿細管`  
3. 本文・選択肢の内容（意味）自体は変更しない。  
4. `correctAnswers` は正答に応じて `['a']` / `['c','e']` のように記載する。  
5. `sourceId` は出典に合わせて設定する（例: `119A75`）。
